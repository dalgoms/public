...
- **비개발자 친화적**: 디자이너/마케터/PM이 코드 없이 사이트 전체 URL 맵을 만들고 내보낼 수 있습니다.
- **Figma 워크플로우 최적화**: Figma, html.to.design 등에 바로 넣을 수 있는 형태로 URL을 정규화·내보내기 합니다.
- **운영 가능한 보안 수준**: SSRF 방어, CORS allowlist, 레이트 리밋, 입력 검증, self-check 스크립트까지 포함해 실제 운영 환경에서도 안전하게 사용할 수 있습니다.
- **인사이트 지향 UI**: Sitemap, 페이지 유형, Figma 준비도, 중요도 탭을 통해 무엇을 먼저 리디자인·개선할지 빠르게 결정할 수 있습니다.

## 어떤 인사이트를 얻을 수 있나요?

- **퍼널·구조 인사이트**: 랜딩 → 제품 → 전환 단계별 URL 분포, 깊이(Depth) 분석, 내부 링크 허브 페이지 파악.
- **콘텐츠·SEO 인사이트**: 블로그/문서/Use-case/사례 페이지 등 카테고리별 URL 커버리지, 쿼리·파라미터 패턴, 중복·Thin 콘텐츠 후보.
- **UX·전환 인사이트**: 전환 페이지 위치(얕은 경로 vs 깊게 묻힌 경로), 쿼리가 많은 URL, 구조적 병목 구간 후보.
- **디자인·리디자인 인사이트**: 템플릿/레이아웃 패밀리 파악, 중요도·준비도를 기반으로 한 리디자인 우선순위 결정.
